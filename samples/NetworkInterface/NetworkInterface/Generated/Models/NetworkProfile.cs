// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Collections.Generic;

namespace NetworkInterface.Models
{
    /// <summary> Network profile resource. </summary>
    public partial class NetworkProfile : NetworkInterface.Models.Resource
    {
        /// <summary> A unique read-only string that changes whenever the resource is updated. </summary>
        public string Etag { get; internal set; }
        /// <summary> List of child container network interfaces. </summary>
        public ICollection<NetworkInterface.Models.ContainerNetworkInterface> ContainerNetworkInterfaces { get; internal set; }
        /// <summary> List of chid container network interface configurations. </summary>
        public ICollection<NetworkInterface.Models.ContainerNetworkInterfaceConfiguration> ContainerNetworkInterfaceConfigurations { get; set; }
        /// <summary> The resource GUID property of the network profile resource. </summary>
        public string ResourceGuid { get; internal set; }
        /// <summary> The provisioning state of the network profile resource. </summary>
        public NetworkInterface.Models.ProvisioningState? ProvisioningState { get; internal set; }
    }
}
